...
Etapp 0–3 from `docs/PROJECT_PLAN.md` are done: Flyway/datalager, Google OAuth2 auth (CORS/CSRF/global error handler), external-recipe scraping (JSoup, SSRF guard, OpenGraph/JSON-LD extraction), and reviews/feed (multiple reviews per recipe, editing, pagination). Etapp 4 (saved recipes) onward has not been started. There is still no `frontend/` — do not assume a `package.json` or Vue project exists until Etapp 5 creates it.
...
- Feed ordering/display uses `created_at` only, not `updated_at` — editing a review does *not* bump it to the top of the feed. `updated_at` is still tracked on `Review` (bumped by `PUT /api/reviews/{id}`) but isn't surfaced by `GET /api/feed` or `GET /api/recipes/{id}/reviews/mine`. This was a judgment call on an ambiguous line in `docs/ARCHITECTURE.md`; revisit if the product actually wants edit-bumps-to-top behavior.
